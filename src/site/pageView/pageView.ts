...
export default class SitePageView extends HTMLElement {
  ...
  connectedCallback() {
    ...
    this.$markdown.modify(($content) => {
      ...
      const links = $content.querySelectorAll('h2 > a[href],h3 > a[href],h4 > a[href],h5 > a[href],h6 > a[href]');
      for (let i = 0; i < links.length; i++) {
        const link = links[i];
        addTooltip(link, () => 'Permalink');
      }
    });
  ...
  async render(changes) {
    if (changes.slug && this.slug) {
      ...
      this.$edit.href = `https://github.com/Pictogrammers/Icon-Site/tree/master/src/${file}`;
    }
    if (changes.icons) {
      this.initIcons = true;
      this.renderIcons();
